...
def generate_session_report(
    ...
    # -- quality gauge + 4 stat cards side by side --
    gauge_w = CW * 0.30
    gauge_d = _draw_gauge(gauge_w, 80, quality, 50.0)
    ...
    story.append(_draw_progress_bar(
        float(CW), 10, comp_perc, _pct_col(comp_perc),
    ...
    if len(ex_agg) >= 1:
        story.append(_section("Exercise Comparison", S))
    ...
    if ex_agg:
        ...
        for key, a in ex_agg.items():
            ...
            # Mini score progress bar for this exercise
            story.append(_draw_progress_bar(
                float(CW - 4*mm), 8, avg / 50 * 100, sc,
