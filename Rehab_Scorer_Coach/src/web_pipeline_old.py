...
from Rehab_Scorer_Coach.src.llm_meralion_old import MeralionLLM
...
class WebRehabPipeline:
    ...
    def reset(self, threshold=30.0, exercise_name="exercise", cooldown_seconds=10.0, language="english"):
        self.feat_buffer.clear()
        self.threshold = float(threshold)
        self.exercise_name = exercise_name
        self.cooldown_seconds = float(cooldown_seconds)
        ...
        if status == "WRONG" and (now - self.last_llm_time) >= self.cooldown_seconds:
            try:
                ...
                chunks = self.rag.query(
                    query_text=f"How to perform {self.exercise_name} correctly. common mistakes, cues, steps, form cues, mistakes, safety, breathing",
                    exercise=self.exercise_name,
                    k=6
                )
                rag_context = "\n\n".join([f"[{c.source}] {c.text}" for c in chunks])

                # 2) build numeric summary (you already have score/status)
                #numeric_summary = f"form_status={form_status}, frame_score={frame_score:.2f}"

                # 3) call MERaLiON with image + context + language
                if frame_b64.startswith("data:image"):
                    frame_b64 = frame_b64.split(",", 1)[1]   # keep only base64 payload

                feedback_list = self.llm.generate_feedback(
                    exercise_name=self.exercise_name,
                    language=self.language,
                    frame_b64=frame_b64,   # already base64 data URL or raw b64 (match what you send)
